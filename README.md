...
フリーランス案件として、個人事業主のクライアントから「取引先とのLINEグループで自分が連絡を送ったあと、相手から返信が来ているか確認し忘れることが多い」という課題をヒアリングしたことが開発のきっかけです。

クライアントは複数の取引先と並行してやり取りしており、既読・未読だけでは「返信待ちかどうか」を判断しにくく、対応漏れが発生していました。対応漏れが発生するとビジネス機会の損失や信頼低下につながるため、「自分が発言してから一定時間返信がなければ自動でリマインドを飛ばしたい」というニーズが生まれました。

既存のLINEには自動リマインド機能がなく、また外部CRMや通知サービスとの連携はコストや操作の複雑さからクライアントには適していなかったため、普段から使い慣れたGoogle Apps Scriptで完結するシンプルな自動化ツールを設計・開発しました。

## 技術選定

### Google Apps Script (GAS)

- クライアントがGoogleアカウントを既に使用しており、追加のサーバー費用やインフラ管理なしにデプロイできるため採用
- Webhookエンドポイント（`doPost`）と定期実行トリガー（`checkAndSendReminders`）の両方をGAS単体で実現でき、外部サービスへの依存を最小化できる
- スクリプトプロパティによる秘匿情報管理が標準機能として備わっており、セキュリティ面でも非エンジニアのクライアントに引き渡しやすい

### LINE Messaging API

- クライアントの業務連絡がLINEグループ上で完結しているため、ユーザー側の操作変更なしにシステムを導入できる点を優先
- Push Message APIを使うことで、グループへの一方向送信が容易に実現できる
- Webhook受信によりリアルタイムにメッセージイベントを取得でき、ポーリングと組み合わせることで確実な発言検知が可能

### 設計上のトレードオフ

GASの同時実行数制限や実行時間上限があるため、大規模なグループ監視には向きません。本ツールは「1グループ・1ユーザー」の監視という用途に絞ることで、GASの制約内で安定動作を優先する設計としています。Webhookの再送機能をオフにする運用ガイドを明記しているのも、GASの冪等性の欠如を補う対処です。
